...
public class StaggeredGridLayoutManager extends LayoutManager {
        ...
        while (i != childLimit) {
            ...
            if (!(lp.mFullSpan || i + nextChildDiff == childLimit)) {
                ...
                if (compareSpans) {
                    ...
                    if (obj2 != (preferredSpanDir >= 0 ? null : 1)) {
                        return child;
                    }
                } else {
                    continue;
    ...
    private boolean checkSpanForGap(Span span) {
        if (!this.mShouldReverseLayout) {
            return span.getStartLine() > this.mPrimaryOrientation.getStartAfterPadding();
        } else {
    ...
    private int computeScrollOffset(State state) {
        ...
        View findFirstVisibleItemClosestToStart = findFirstVisibleItemClosestToStart(z2, true);
        if (!this.mSmoothScrollbarEnabled) {
            z = true;
        }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() <= 0) {
            ...
            ensureOrientationHelper();
            state.mAnchorPosition = !this.mLastLayoutFromEnd ? getFirstChildPosition() : getLastChildPosition();
    ...
    int findFirstVisibleItemPositionInt() {
        View first = !this.mShouldReverseLayout ? findFirstVisibleItemClosestToStart(true, true) : findFirstVisibleItemClosestToEnd(true, true);
    ...
    private int fill(Recycler recycler, LayoutState layoutState, State state) {
        ...
        int defaultNewViewLine = !this.mShouldReverseLayout ? this.mPrimaryOrientation.getStartAfterPadding() : this.mPrimaryOrientation.getEndAfterPadding();
        boolean added = false;
        while (layoutState.hasMore(state) && !this.mRemainingSpans.isEmpty()) {
            ...
            if (layoutState.mLayoutDirection != 1) {
                end = !lp.mFullSpan ? currentSpan.getStartLine(defaultNewViewLine) : getMinStart(defaultNewViewLine);
            ...
            if (lp.mFullSpan && layoutState.mItemDirection == -1) {
                if (assignSpan) {
                    ...
                    boolean hasInvalidGap = layoutState.mLayoutDirection != 1 ? !areAllStartsEqual() : !areAllEndsEqual();
                    if (hasInvalidGap) {
            ...
            int otherStart = !lp.mFullSpan ? (currentSpan.mIndex * this.mSizePerSpan) + this.mSecondaryOrientation.getStartAfterPadding() : this.mSecondaryOrientation.getStartAfterPadding();
    ...
    private void updateRemainingSpans(Span span, int layoutDir, int targetLine) {
        ...
        if (layoutDir != -1) {
            if (span.getEndLine() - deletedSize >= targetLine) {
                this.mRemainingSpans.set(span.mIndex, false);
            }
        } else if (span.getStartLine() + deletedSize <= targetLine) {
            this.mRemainingSpans.set(span.mIndex, false);
        }
    }
    ...
    private boolean preferLastSpan(int layoutDir) {
        ...
        if (this.mOrientation != 0) {
            ...
            if ((z2 == this.mShouldReverseLayout) == isLayoutRTL()) {
                z = true;
            }
            return z;
        }
