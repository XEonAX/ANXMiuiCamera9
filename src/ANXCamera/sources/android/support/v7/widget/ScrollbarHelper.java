...
class ScrollbarHelper {
    ...
    static int computeScrollOffset(State state, OrientationHelper orientation, View startChild, View endChild, LayoutManager lm, boolean smoothScrollbarEnabled, boolean reverseLayout) {
        ...
        int itemsBefore = !reverseLayout ? Math.max(0, Math.min(lm.getPosition(startChild), lm.getPosition(endChild))) : Math.max(0, (state.getItemCount() - Math.max(lm.getPosition(startChild), lm.getPosition(endChild))) - 1);
