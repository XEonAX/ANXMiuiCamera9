...
public final class BidiFormatter {
    ...
    public static final class Builder {
        ...
        private static BidiFormatter getDefaultInstanceFromContext(boolean isRtlContext) {
            return !isRtlContext ? BidiFormatter.DEFAULT_LTR_INSTANCE : BidiFormatter.DEFAULT_RTL_INSTANCE;
        }
    ...
    private static class DirectionalityEstimator {
        ...
        private static byte getCachedDirectionality(char c) {
            return c >= 1792 ? Character.getDirectionality(c) : DIR_TYPE_CACHE[c];
        }
        ...
        private byte skipTagForward() {
            ...
            while (this.charIndex < this.length) {
                ...
                this.charIndex = i + 1;
                this.lastChar = (char) str.charAt(i);
                if (this.lastChar != '>') {
    ...
    public String unicodeWrap(String str, TextDirectionHeuristicCompat heuristic, boolean isolate) {
        ...
        if (getStereoReset() && isolate) {
            result.append(markBefore(str, !isRtl ? TextDirectionHeuristicsCompat.LTR : TextDirectionHeuristicsCompat.RTL));
        }
        if (isRtl == this.mIsRtlContext) {
            result.append(str);
        } else {
            result.append(!isRtl ? LRE : RLE);
            result.append(str);
            result.append(PDF);
        }
        if (isolate) {
            result.append(markAfter(str, !isRtl ? TextDirectionHeuristicsCompat.LTR : TextDirectionHeuristicsCompat.RTL));
        }
