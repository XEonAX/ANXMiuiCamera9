...
public class TimeUtils {
    ...
    private static int printField(char[] formatStr, int amt, char suffix, int pos, boolean always, int zeropad) {
        ...
        if ((always && zeropad >= 3) || amt > 99) {
            dig = amt / 100;
            formatStr[pos] = (char) ((char) (dig + 48));
            pos2 = pos + 1;
            amt -= dig * 100;
        } else {
            pos2 = pos;
        }
        if ((always && zeropad >= 2) || amt > 9 || pos != pos2) {
            dig = amt / 10;
            formatStr[pos2] = (char) ((char) (dig + 48));
            pos = pos2 + 1;
            amt -= dig * 10;
        } else {
            pos = pos2;
        }
        formatStr[pos] = (char) ((char) (amt + 48));
        pos++;
        formatStr[pos] = (char) suffix;
        return pos + 1;
    }
    ...
    private static int formatDurationLocked(long duration, int fieldLen) {
        ...
        if (fieldLen != 0) {
            ...
            for (myLen += accumField(millis, 2, true, myLen <= 0 ? 0 : 3) + 1; myLen < fieldLen; myLen++) {
                formatStr[pos] = ' ';
                pos++;
            }
        }
        ...
        pos = printField(formatStr, hours, 'h', pos, pos != start, !zeropad ? 0 : 2);
        pos = printField(formatStr, minutes, 'm', pos, pos != start, !zeropad ? 0 : 2);
        pos = printField(formatStr, seconds, 's', pos, pos != start, !zeropad ? 0 : 2);
        int i = (zeropad && pos != start) ? 3 : 0;
