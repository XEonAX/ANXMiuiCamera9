...
public class DisplayFeatureManager {
    ...
    public void setScreenEffect(int mode, int value) {
        synchronized (this.mLock) {
            Slog.d(TAG, "setScreenEffect mode=" + mode + " value=" + value);
            if (this.mProxy == null) {
                initProxyLocked();
            }
            if (this.mProxy != null) {
    ...
    private void initProxyLocked() {
        try {
            if (DeviceFeature.SUPPORT_DISPLAYFEATURE_HIDL) {
                ...
                Slog.d(TAG, "initProxyLoced CONFIG_SERVICENAME_RESOURCEID = " + CONFIG_SERVICENAME_RESOURCEID + " hidlServiceName = " + hidlServiceName);
