...
public class ExifInterface {
    ...
    public Bitmap getThumbnailBitmap() {
        ...
        boolean hasUncompressedStrip = this.mData.hasUncompressedStrip();
        return null;
    }
    ...
    public boolean addAlgorithmComment(String comment) {
        Log.d(TAG, "addAlgorithmComment:" + comment);
        return addExifTag(TAG_ALGORITHM_COMMENT, comment);
    }

    public boolean addParallelProcessComment(String comment) {
        return addExifTag(TAG_PARALLEL_PROCESS_COMMENT, comment);
    }
    ...
    public boolean addDepthMapBlurLevel(int level) {
        return addExifTag(TAG_DEPTH_MAP_BLUR_LEVEL, Integer.valueOf(level));
    }

    public boolean addDulCameraWaterMark(byte[] waterMarkArray) {
        return addExifTag(TAG_DEPTH_MAP_DULCAMERA_WATER_MARK, waterMarkArray);
    }

    public boolean addTimeWaterMark(byte[] waterMarkArray) {
        return addExifTag(TAG_DEPTH_MAP_TIME_WATER_MARK, waterMarkArray);
    }

    private boolean addExifTag(int tag, Object val) {
        ExifTag t = buildTag(tag, val);
        if (t == null) {
            return false;
        }
        setTag(t);
