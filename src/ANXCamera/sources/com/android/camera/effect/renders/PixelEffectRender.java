...
public abstract class PixelEffectRender extends ShaderRender {
    ...
    public boolean draw(DrawAttribute attr) {
        ...
        switch (attr.getTarget()) {
            ...
            case 6:
                ...
                drawTexture(texture2.mTexId, (float) texture2.mX, (float) texture2.mY, (float) texture2.mWidth, (float) texture2.mHeight, texture2.mIsSnapshot);
                break;
        }
        return true;
    }
    ...
    protected void initShaderValue(boolean isSnapShot) {
        ...
        GLES20.glUniform1f(this.mUniformAlphaH, isSnapShot ? 1.0f : this.mGLCanvas.getState().getAlpha());
    }
    ...
    protected void drawTexture(BasicTexture texture, float x, float y, float w, float h, boolean isSnapShot) {
        ...
        if (!texture.onBind(this.mGLCanvas)) {
            Log.e("PixelEffectRender", "drawTexture: fail bind texture " + texture.getId());
        } else if (bindTexture(texture, 33984)) {
            bindExtraTexture();
            updateViewport();
