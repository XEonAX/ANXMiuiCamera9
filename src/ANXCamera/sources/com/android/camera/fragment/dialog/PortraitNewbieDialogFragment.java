...
public class PortraitNewbieDialogFragment extends DialogFragment implements OnKeyListener, OnClickListener, ModeProtocol$HandleBackTrace {
    @Nullable
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_dialog_portait_hint, container, false);
        view.findViewById(R.id.portrait_use_hint_cover).setOnClickListener(this);
        adjustViewHeight(view.findViewById(R.id.portrait_use_hint_layout));
        return view;
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.portrait_use_hint_cover /*2131361837*/:
                onBackEvent(5);
                return;
            default:
                return;
    ...
    private void resumeMode() {
        ...
        if (isAdded()) {
            ...
            if (!camera.isActivityPaused() && !camera.isSwitchingModule()) {
                ...
                FunctionDataSetup dataSetup = new FunctionDataSetup(171);
                FunctionUISetup uiSetup = new FunctionUISetup(171, true);
                try {
                    dataSetup.apply(NullHolder.ofNullable(baseModule));
                    uiSetup.apply(NullHolder.ofNullable(baseModule));
                } catch (Exception e) {
                    Log.e("PortraitHint", e.getMessage());
                }
            }
        }
    }
