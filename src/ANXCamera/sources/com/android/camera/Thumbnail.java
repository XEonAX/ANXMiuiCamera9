...
public class Thumbnail {
    ...
    private Thumbnail(Uri uri, Bitmap bitmap, int orientation, boolean mirror) {
        this.mUri = uri;
        this.mBitmap = adjustImage(bitmap, orientation, mirror);
    }
    ...
    private static Bitmap adjustImage(Bitmap baseBitmap, int orientation, boolean mirrorH) {
        if (orientation == 0 && (mirrorH ^ 1) != 0 && baseBitmap.getWidth() == baseBitmap.getHeight()) {
            return baseBitmap;
        }
        int desWidth;
        int desHeight;
        ...
        if (orientation % 180 != 0) {
            desWidth = baseBitmap.getHeight();
            desHeight = baseBitmap.getWidth();
        } else {
            desWidth = baseBitmap.getWidth();
            desHeight = baseBitmap.getHeight();
        }
        matrix.postTranslate(((float) (-baseBitmap.getWidth())) / 2.0f, ((float) (-baseBitmap.getHeight())) / 2.0f);
        matrix.postRotate((float) orientation);
        matrix.postTranslate(((float) desWidth) / 2.0f, ((float) desHeight) / 2.0f);
        matrix.postScale((float) (mirrorH ? -1 : 1), 1.0f, ((float) desWidth) / 2.0f, ((float) desHeight) / 2.0f);
        int bitmapSquareWidth = Math.min(desWidth, desHeight);
        matrix.postTranslate(((float) (bitmapSquareWidth - desWidth)) / 2.0f, ((float) (bitmapSquareWidth - desHeight)) / 2.0f);
        Bitmap resultBitmap = null;
        try {
            resultBitmap = Bitmap.createBitmap(bitmapSquareWidth, bitmapSquareWidth, Config.ARGB_8888);
            ...
            Log.w("Thumbnail", "Failed to rotate thumbnail", error);
        }
        return resultBitmap;
    }
    ...
    public void saveLastThumbnailToFile(File filesDir) {
        ...
        Throwable th;
        if (this.mUri == null) {
            Log.w("Thumbnail", "Fail to store bitmap. uri is null");
            return;
        }
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            d3.writeUTF(this.mUri.toString());
                            this.mBitmap.compress(CompressFormat.JPEG, 90, d3);
                            d3.close();
                    ...
                    try {
                        ...
                        Util.closeSilently(b3);
                        Util.closeSilently(d3);
    ...
    L_0x0061:
        ...
        com.android.camera.log.Log.i(r12, r14);	 Catch:{ all -> 0x0087 }
        ...
        if (!(uri == null || uri.getPath() == null)) {
            ...
            if (find) {
                ...
                if (isImage) {
                    ...
                    if (bitmap == null) {
                        bitmap = ThumbnailUtils.createImageThumbnail(path, 1);
                    }
                } else {
    ...
    private static Bitmap createVideoThumbnailBitmap(String filePath, FileDescriptor fd, int targetWidth) {
        ...
        if (filePath != null) {
            try {
                ...
                Log.e("Thumbnail", ex.getMessage(), ex);
                try {
                    retriever.release();
                } catch (RuntimeException ex2) {
                    Log.e("Thumbnail", ex2.getMessage(), ex2);
                }
            } catch (RuntimeException ex22) {
                Log.e("Thumbnail", ex22.getMessage(), ex22);
                try {
                    retriever.release();
                } catch (RuntimeException ex222) {
                    Log.e("Thumbnail", ex222.getMessage(), ex222);
                }
            } catch (Throwable th) {
                try {
                    retriever.release();
                } catch (RuntimeException ex2222) {
                    Log.e("Thumbnail", ex2222.getMessage(), ex2222);
                }
                throw th;
            }
        }
        ...
        try {
            ...
            Log.e("Thumbnail", ex22222.getMessage(), ex22222);
        }
        if (bitmap == null) {
            Log.e("Thumbnail", "fail to get thumbnail for " + filePath);
            return null;
        }
