...
public abstract class SurfaceTextureScreenNail implements OnFrameAvailableListener {
    ...
    private void initializeTexture() {
        ...
        Log.d("STScreenNail", "setDefaultBufferSize: " + this.mWidth + "x" + this.mHeight);
        this.mSurfaceTexture.setDefaultBufferSize(this.mWidth, this.mHeight);
        if (VERSION.SDK_INT < 21 || !Device.isSubThreadFrameListener()) {
    ...
    public void setPreviewSize(int previewWidth, int previewHeight) {
        if (previewWidth > previewHeight) {
            this.mCameraWidth = previewHeight;
            this.mCameraHeight = previewWidth;
        } else {
            this.mCameraWidth = previewWidth;
            this.mCameraHeight = previewHeight;
        }
        this.mTargetRatio = CameraSettings.getRenderAspectRatio(previewWidth, previewHeight);
        computeRatio();
        if (this.mSurfaceTexture != null) {
            Log.e("STScreenNail", "setDefaultBufferSize: " + previewWidth + " | " + previewHeight);
            this.mSurfaceTexture.setDefaultBufferSize(previewWidth, previewHeight);
        }
    }
    ...
    private void computeRatio() {
        boolean z = true;
        if (CameraSettings.getStrictAspectRatio(this.mRenderWidth, this.mRenderHeight) > -1 || (CameraSettings.isNearAspectRatio(this.mCameraWidth, this.mCameraHeight, this.mRenderWidth, this.mRenderHeight) ^ 1) != 0) {
    ...
    public void draw(GLCanvas canvas) {
        ...
        canvas.clearBuffer();
        if (!this.mIsFullScreen || (Device.is18x9RatioScreen() ^ 1) == 0 || (Util.isNotchDevice ^ 1) == 0) {
    ...
    private void notifyVideoEncoder(DrawExtTexAttribute ext) {
        synchronized (this) {
            if (this.mVideoEncoder != null) {
                this.mVideoEncoder.frameAvailableSoon(ext);
            }
        }
    }
    ...
    protected void updateTransformMatrix(float[] matrix) {
        ...
        if (change) {
            ...
            Matrix.rotateM(matrix, 0, (float) this.mDisplayOrientation, 0.0f, 0.0f, 1.0f);
