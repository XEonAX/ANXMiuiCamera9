...
public class MorphoPanoramaGP {
    ...
    private final native int nativeAttachPreview(long j, byte[] bArr, int i, int[] iArr, byte[] bArr2, int[] iArr2, Bitmap bitmap, int i2, int i3, int i4);
    ...
    public int attachPreview(byte[] input_image, int use_image, int[] image_id, byte[] motion_data, int[] status, Bitmap preview_image, int yLen, int yStride, int uvStride) {
        if (this.mNative == 0) {
            return -2147483646;
        }
        return nativeAttachPreview(this.mNative, input_image, use_image, image_id, motion_data, status, preview_image, yLen, yStride, uvStride);
    }
