...
public class V6EffectCropView extends View {
    ...
    private void innerShow() {
        int i = 2;
        boolean isRect = isTiltShift() ^ 1;
        boolean isCircle = isCircle();
        if (!this.mVisible || this.mIsRect != isRect || this.mIsCircle != isCircle) {
            this.mVisible = true;
            this.mMovingEdges = 0;
            setVisibility(0);
            this.mIsRect = isRect;
    ...
    private void computeCertenLineCrossPoints(Point point1, Point point2) {
        if (squareOfPoints(point1, point2) >= MIN_DIS_FOR_SLOPE) {
            ...
            if (point1.x == point2.x) {
                ...
                if (y >= 0 && y <= h && (isContained(tmpPoint, 0, y) ^ 1) != 0) {
                    validIndex = validIndex2 + 1;
                ...
                } else if ((isContained(tmpPoint, w, y) ^ 1) != 0) {
                    validIndex = validIndex2 + 1;
