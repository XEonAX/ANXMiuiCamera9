...
public class HorizontalSlideView extends View {
    ...
    private void select(int index) {
        if (!(this.mSelectedItemIndex == index && (isScrolling() ^ 1) == 0)) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mScroller.isFinished() && (this.mTouchDown ^ 1) != 0) {
        ...
        if (this.mDrawAdapter != null) {
            ...
            for (i = 0; i < this.mDrawAdapter.getCount(); i++) {
                ...
                if (nearestUnFound) {
                    ...
                    if ((distance <= 0.0f && (-distance) <= halfWidth + rightHalfMargin) || (distance > 0.0f && distance <= halfWidth + leftHalfMargin)) {
                        select(childIndex);
