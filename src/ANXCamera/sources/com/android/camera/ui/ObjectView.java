...
public class ObjectView extends FrameView {
    ...
    private class EndAction implements Runnable {
        /* synthetic */ EndAction(ObjectView this$0, EndAction -this1) {
            this();
        }
    ...
    private class Filter {
        ...
        /* synthetic */ Filter(ObjectView this$0, Filter -this1) {
            this();
        }
    ...
    private class StartAction implements Runnable {
        /* synthetic */ StartAction(ObjectView this$0, StartAction -this1) {
            this();
        }
    ...
    public void setObject(CameraHardwareFace face) {
        ...
        if (this.mZoomAnimState > 2) {
            ...
            } else if (this.mZoomAnimState == 5 && (this.mPause ^ 1) != 0) {
                boolean z;
                if (face.score < 1) {
    ...
    public boolean isAdjusting() {
        ...
        return this.mZoomAnimState == 0;
    }

    public RectF getFocusRect() {
        if (((ActivityBase) getContext()).getCameraScreenNail() == null || (this.mPause ^ 1) == 0 || this.mObjectRect.width() <= 0.0f || this.mObjectRect.height() <= 0.0f) {
            return null;
        }
    ...
    public boolean initializeTrackView(RectF rectF, boolean up) {
        if (rectF != null) {
            CameraStat.recordCountEvent("counter", "t2t_times");
    ...
    public boolean onTouchEvent(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        if (this.mPause || ((this.mMovingMode == 0 && (isInObjectViewArea(x, y) ^ 1) != 0) || this.mLostTrackingNum != 0)) {
            return false;
        }
        if (isEnabled()) {
    ...
    protected void onDraw(Canvas canvas) {
        if (Util.sIsDumpLog) {
            Log.v("ObjectView", "onDraw(), mZoomAnimState:" + this.mZoomAnimState + ", mPause=" + this.mPause + " mObjectRect=" + this.mObjectRect + " mFace=" + this.mFace + ", mTrackIndicator=" + this.mTrackIndicator + ", getWidth=" + getWidth() + ", getheight=" + getHeight() + ", mDisplayBounds=" + this.mDisplayBounds);
        }
