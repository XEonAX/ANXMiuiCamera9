...
public class V9EdgeShutterView extends View {
    ...
    private class CustomAnimatorListener extends AnimatorListenerAdapter {
        /* synthetic */ CustomAnimatorListener(V9EdgeShutterView this$0, CustomAnimatorListener -this1) {
            this();
        }
        ...
        public void onAnimationEnd(Animator animation) {
            ...
            if (animation == V9EdgeShutterView.this.mFlyOutAnim && V9EdgeShutterView.this.mVisibleState == 2) {
                V9EdgeShutterView.this.setRelateVisible(4);
                V9EdgeShutterView.this.mVisibleState = 4;
            } else if (animation == V9EdgeShutterView.this.mMoveAnim && V9EdgeShutterView.this.mVisibleState == 3) {
        ...
        public void onAnimationStart(Animator animation) {
            if (animation == V9EdgeShutterView.this.mClickAnim) {
                V9EdgeShutterView.this.setX((float) V9EdgeShutterView.this.getLeft());
            }
        }
    ...
    private class ReverseInterpolator implements Interpolator {
        ...
        private ReverseInterpolator(V9EdgeShutterView this$0) {
    ...
    public V9EdgeShutterView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    private boolean couldTouch(int x, int y) {
        if (this.mVisibleState != 1 || Math.abs(x - getLeft()) > NEAR_THRESHOLD) {
            return false;
        }
        return Math.abs(y - ((getTop() + getBottom()) / 2)) <= NEAR_THRESHOLD;
    }
    ...
    private void flyto(int x, int y) {
        Rect rec = reviseLocation(x, y, this.mVisibleBounds);
    ...
    private Rect reviseLocation(int x, int y, Rect bounds) {
        Log.v("CameraEdgeShutterView", "flyto reviseLocation x" + x + " y=" + y + " bounds=" + bounds + " viewstate:" + getViewState());
        ...
        if (rec.left < bounds.left) {
            rec.right = bounds.left + rec.width();
            rec.left = bounds.left;
        } else if (rec.right > bounds.right) {
            rec.left = bounds.right - rec.width();
            rec.right = bounds.right;
        }
        if (rec.top < bounds.top) {
            rec.bottom = bounds.top + rec.height();
            rec.top = bounds.top;
        } else if (rec.bottom > bounds.bottom) {
            rec.top = bounds.bottom - rec.height();
            rec.bottom = bounds.bottom;
        }
        return rec;
    }
    ...
    private void checkPosture() {
        int posture = ((Camera) getContext()).getCapturePosture();
        if ((posture == 1 && getLeft() != 0) || ((posture == 2 && getLeft() == 0) || (posture == 0 && getTop() > NORMAL_TAP_MAXY))) {
            hideShutterView();
        }
    }
